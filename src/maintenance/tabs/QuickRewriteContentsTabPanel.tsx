...
function QuickRewriteContentsTabPanel({ data, loading }: { data: RunInfo[]; loading: boolean }) {
  const { pageSize: tablePageSize, bytesStringBase2 } = useAppContext();
  return (
    <TabsPanel value="quick-rewrite-contents">
      <DataGrid
        records={data}
        loading={loading}
        idAccessor="start"
...
export default QuickRewriteContentsTabPanel;
